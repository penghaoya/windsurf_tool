...
class AuthService {
  ...
  readCachedRateLimit() {
    try {
      ...
      if (cachedPlan) {
        ...
        } catch (e) { console.log('WAM: [CACHED_RL] cachedPlanInfo parse error:', e.message); }
      }
      ...
      for (const [key, val] of Object.entries(data)) {
        ...
        } catch (e) { console.log(`WAM: [CACHED_RL] key=${key} parse error: ${e.message}, raw=${String(val).substring(0, 100)}`); }
      }
  ...
  writeModelSelection(modelUid) {
    try {
      ...
      console.log(`WAM: writeModelSelection(${modelUid}) => ${ok ? 'OK' : 'FAIL'}`);
      return ok;
    } catch (e) {
