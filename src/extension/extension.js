...
function _activate(context) {
  ...
  _logInfo(
    "BOOT",
    `号池引擎就绪 v11.0 — ${accounts.length}账号, proxy=${auth.getProxyStatus().mode}, win=${_getActiveWindowCount()}, tabs=${_cascadeTabCount}${_burstMode ? " BURST" : ""}, gates=4(Q/W/Model/Tier), detect=L1-2s/L3-10s/L5-3s(T)/45s, budget=T1M:${OPUS_THINKING_1M_BUDGET}/T:${OPUS_THINKING_BUDGET}/R:${OPUS_REGULAR_BUDGET}`,
  );
}
...
function _clearCachedPlanInfo() {
  try {
    ...
    if (dbDeleteKey(dbPath, 'windsurf.settings.cachedPlanInfo')) {
      ...
      _logWarn("CACHE", "cache clear skipped (non-critical)");
    }
  } catch (e) {
  ...
  try {
    ...
    try {
      if (dbUpdateKeys(dbPath, pairs)) {
        ...
        _logWarn("FP", "state.vscdb skip (non-critical)");
      }
    } catch (e) {
