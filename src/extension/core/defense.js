...
export async function _probeCapacity() {
  ...
  // v24.0: re-enable L5 for Trial accounts. Now that checkRateLimitCapacity
  // passes modelUid, the server returns precise per-model messagesRemaining
  // even for Trial (parity with windsurf-pool). The previous skip was a
  // workaround for the "always -1/-1" bug caused by missing modelUid.
